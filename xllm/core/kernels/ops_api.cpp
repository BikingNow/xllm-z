...
torch::Tensor causal_conv1d_update(CausalConv1dUpdateParams& params) {
  ...
  const bool need_fp16_cast = (original_dtype == c10::ScalarType::BFloat16);

  // --- dtype conversion: bf16 → fp16 (TileLang kernel requires fp16) ---
  auto x_work = params.x.contiguous();
  auto weight_work = params.weight.contiguous();
  // Clone conv_state for bf16 conversion (.to creates a new tensor anyway),
  // but for fp16 input keep the original so kernel in-place writeback
  // propagates directly to params.conv_state without an extra copy.
  auto conv_state_work = need_fp16_cast ? params.conv_state.contiguous().clone()
                                        : params.conv_state.contiguous();

  if (need_fp16_cast) {
    x_work = x_work.to(torch::kFloat16);
    weight_work = weight_work.to(torch::kFloat16);
    conv_state_work = conv_state_work.to(torch::kFloat16);
  }
  ...
  if (need_fp16_cast && bias_work.scalar_type() != c10::ScalarType::Half) {
    bias_work = bias_work.to(torch::kFloat16);
  }
